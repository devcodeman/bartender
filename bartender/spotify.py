...
try:
  ...
  print("An Environment Variable for Spotify is incorrrect. Please verify set CLIENT_ID, CLIENT_SECRET and CLIENT_REDIRECT_URI.\n")

class Spotify:
  def __init__(self, deviceName:str, nospotify):
    self._scope       = "user-read-playback-state,user-modify-playback-state"
    if not nospotify:
      self._credentials = SpotifyOAuth(client_id=CLIENT_ID, client_secret=CLIENT_SECRET, redirect_uri=CLIENT_REDIRECT_URI, scope=self._scope, open_browser=True)
      self.spotify      = spotipy.Spotify(auth_manager=self._credentials)
    else:
  ...
  def establishConnection(self, maxAttempts=3) -> Any:
    currentRetries = 1
